...
params = {}
params['path'] = 'F:/data/TEST104.csv'
params['opath'] = 'F:/pyadd/test/test_fea1.csv'

#params = {}
#params['data_path'] = 'F:/data'
#params['save_path'] = 'F:/pyadd/test/test_fea1'
...
try:
    ...
    columns_list1 = [a + '_vibD' for a in columns_list]
    columns_list2 = [a + '_vibF' for a in columns_list]
    #label_columns = ['lable']
    columns_list_final = columns_list1 + columns_list2#+label_columns
    
    def feature_get(filepath,info):
        t=0
        global df1
        name = info.split('.')[0]
        total = len(open(filepath).readlines())
        data=[name]
        df2=pd.DataFrame(data,columns=['label'])
        while t<total:
            col = ['vib_d','vib_f','3']
            dfs = pd.DataFrame(pd.read_csv(filepath,header=0,names = col))
            if t>total-500:
                df = dfs.loc[t-500:total,['vib_d','vib_f']]
            else:
                df = dfs.loc[t:(t+500),['vib_d','vib_f']]
            feature_list = []
            for i in df.columns:
